...
(async () => {
  ...
  await page.goto('http://127.0.0.1:8766/index.html?v=24');
  ...
  await tab('graphing');
  assert.equal(await page.locator('#desmos').getAttribute('src'), 'https://www.desmos.com/calculator');
  ...
  await offlinePage.goto('http://127.0.0.1:8766/index.html?v=24');
  ...
  const cached = await offlinePage.evaluate(async () => {
    const cache = await caches.open('math-aac-v24'); return (await cache.keys()).map(r => new URL(r.url).pathname + new URL(r.url).search);
  });
  assert.ok(cached.includes('/math.js?v=24')); assert.ok(cached.includes('/speech.js?v=24')); assert.ok(!cached.includes('/api/tts-status'));
